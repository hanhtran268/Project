...
def render_tab4():
    ...
    # Plotting the histogram
        ...
        ax.hist(simulated_df.iloc[-1, ], bins=30,
                color='blue', alpha=0.7, label='Simulated Prices')
        ax.axvline(simulated_df.iloc[-1, ].median(), color='green',
                   linestyle='dashed', linewidth=2, label='Median')
        ax.axvline(np.percentile(
            simulated_df.iloc[-1, ], 5), color='red', linestyle='dashed', linewidth=2, label='VaR (5%)')
        ...
        # Show the hist
        st.subheader("Value at Risk (VaR) at 95%")
        st.pyplot(fig)
